...
function addClasses(elem, activeClass, classPref, datasetName) {
    elem.parentElement.querySelector('.' + activeClass).classList.remove(activeClass); // смена активного класса переключателя
    elem.classList.add(activeClass);

    const clearClasses = book.className.split(' ').filter(bookClass => bookClass.indexOf(classPref) != -1); // массив из ранее применённых аналогичных классов book
    clearClasses.forEach((clearClass) => book.classList.remove(clearClass)); // удаляем предыдущие аналогичные классы book

    const bookClass = elem.dataset[datasetName] ? classPref + '-' + elem.dataset[datasetName] : ''; // если dataset со стилем задан, делаем название нового класса
